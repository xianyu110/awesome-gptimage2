...
### GPT-Image-2 中文提示词实战手册

把 GPT-Image-2 的实测经验整理成可复制、可改写、可落地的提示词方法论。
...
1. 先看 [提示词框架](#提示词框架)，理解「任务类型 + 主体描述 + 风格定义 + 技术参数 + 输出规格」。
2. 到 [提示词合集](#提示词合集) 选择最接近的场景，复制后替换主体、文案、比例和风格。
3. 用 [高级技巧](#高级技巧) 做二次迭代，优先优化文字内容、参考图和画面比例。
...
- **电商 / 运营团队**：复用商品详情页、主图、活动海报、种草图模板。
- **内容创作者**：制作信息图、漫画分镜、封面图、公众号与小红书素材。
- **Prompt 玩家**：系统对比不同场景下的结构化提示词写法。
- **开发者 / 产品经理**：了解图像生成模型在 API、界面和工作流里的落地方式。

## 推荐阅读路线

| 目标 | 建议阅读 |
|------|----------|
| 5 分钟上手 | [快速开始](#快速开始) → [提示词框架](#提示词框架) → 复制一个场景模板 |
| 做电商图 | [电商与产品](#一电商与产品) → [图片编辑与参考](#八图片编辑与参考) |
| 做内容传播 | [知识科普与信息图](#四知识科普与信息图) → [艺术创作](#九艺术创作) |
| 做真实照片 | [真实摄影](#六真实摄影) → [photorealistic 技巧](#技巧-1photorealistic-是万能钥匙) |
| 做复杂项目 | [Thinking 模式](#thinking-模式思考模式) → [补充案例提示词](#十一补充案例提示词) |
...
**如果这个提示词库帮你节省了调参时间，欢迎给个 Star。**

Made with GPT-Image-2 | Last updated: 2026-04-25
